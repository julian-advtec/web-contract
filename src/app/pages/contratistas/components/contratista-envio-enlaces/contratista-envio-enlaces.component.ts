...
interface ContratistaEstadisticas {
  total: number;
  conEmail: number;
  sinEmail: number;
  activos: number;
...
@Component({
  ...
  imports: [
    ...
    FormsModule,
    RouterModule,
    LoadingSpinnerComponent, // ✅ AGREGADO
    PaginationComponent,      // ✅ AGREGADO
  ],
...
export class ContratistaEnvioEnlacesComponent implements OnInit, OnDestroy {
  ...
  paginatedItems: ContratistaSeleccionado[] = [];
  ...
  // Estadísticas
  estadisticas: ContratistaEstadisticas = {
    total: 0,
    conEmail: 0,
    sinEmail: 0,
    activos: 0,
  ...
  updatePagination(): void {
    ...
    this.paginatedItems = this.getContratistasPaginated();
  }
  ...
  toggleSeleccionTodos(): void {
    const visibles = this.paginatedItems;
