// asesor-gerencia-pending-list.component.ts
...
export class AsesorGerenciaPendingListComponent implements OnInit, OnDestroy {
            ...
            .subscribe({
                next: (docs: DocumentoGerencia[]) => {
                    ...
                    if (docs && docs.length > 0) {
                        console.log('[Pendientes Gerencia] Primer documento:', docs[0]);
                        console.log('[Pendientes Gerencia] disponible:', docs[0]?.disponible);
                        console.log('[Pendientes Gerencia] tesoreroAsignado:', docs[0]?.tesoreroAsignado);
                        console.log('[Pendientes Gerencia] enMiRevision:', docs[0]?.enMiRevision);
                    }
    ...
    // ✅ CORREGIDO: No usar tesoreroAsignado para disponibilidad
    ...
    private procederTomarDocumento(doc: DocumentoGerencia): void {
        ...
        this.asesorGerenciaService.tomarDocumento(doc.id!)
            ...
            .subscribe({
                next: (response: any) => {
                    console.log('[TOMAR GERENCIA] Respuesta:', response);
